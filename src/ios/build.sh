...
build_plugin_structure() {
	...
	if ls "$path"/EmbeddedFrameworks/*.framework 1> /dev/null 2>&1; then
		...
		for f in "$path"/EmbeddedFrameworks/*.framework; do
			...
			if [[ $(file "$SRC_BIN" | grep -c "ar archive") -ne 0 ]]; then
				...
				echo " + $FRAMEWORK_NAME: embedding dynamic framework"
	...
	if ls "$path"/libs/*.a 1> /dev/null 2>&1; then
		echo "Copying embedded libraries for $PLATFORM:"

		for f in "$path"/libs/*.a; do
			LIB_NAME=$(basename "$f")
			SRC_BIN="$f"

			if [[ $(file "$SRC_BIN" | grep -c "ar archive") -ne 0 ]]; then
				echo " - $LIB_NAME: is a static library, extracting."

				DEST_BIN="$PLUGIN_DEST/$LIB_NAME"
				lipo "$SRC_BIN" $ARCH -o "$DEST_BIN.tmp"
				$(xcrun -f bitcode_strip) "$DEST_BIN.tmp" -r -o "$DEST_BIN"
				rm "$DEST_BIN.tmp"
			fi
